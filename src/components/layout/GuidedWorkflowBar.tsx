...
export const GuidedWorkflowBar: React.FC<{ onOpenHelp?: () => void }> = ({ onOpenHelp }) => {
  ...
  return (
    <div className="bg-white border-b border-[#E4E7EC] px-4 py-2.5 flex items-center justify-between gap-4 select-none shrink-0 shadow-xs z-20">
      ...
      {/* Center: Simplified Progress on Mobile, Full Stepper on Desktop */}
      <div className="flex items-center gap-1 sm:gap-2">
        {/* Mobile Compact Indicator */}
        <div className="flex md:hidden items-center gap-1.5 px-2.5 py-1 rounded-full bg-[#EEF4FF] text-[#2563EB] text-xs font-semibold">
          <span className="w-5 h-5 rounded-full bg-[#2563EB] text-white flex items-center justify-center text-[10px] font-bold">
            {activeIndex + 1}
          </span>
          <span className="truncate max-w-[110px] xs:max-w-[160px]">{currentStage.label}</span>
          <span className="text-[10px] text-[#2563EB]/60 font-normal">({activeIndex + 1}/8)</span>
        </div>
          ...
          {WORKFLOW_STAGES.map((stage, idx) => {
            ...
            return (
              ...
              >
                ...
                <span className={`hidden lg:inline ${isCurrent ? 'text-[#2563EB]' : ''}`}>{stage.label}</span>
              </button>
            );
          })}
        </div>
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
        {onOpenHelp && (
          <button
            onClick={onOpenHelp}
            className="p-2 min-h-[40px] min-w-[40px] rounded-lg text-[#667085] hover:text-[#172033] hover:bg-[#F9FAFB] border border-transparent hover:border-[#E4E7EC] flex items-center justify-center transition-colors"
        ...
        <button
          ...
          className={`px-2.5 sm:px-4 py-2 min-h-[40px] rounded-lg text-xs font-semibold flex items-center gap-1 sm:gap-1.5 transition-all shadow-xs ${
            activeIndex === WORKFLOW_STAGES.length - 1
              ? 'bg-[#12B76A] hover:bg-[#0E9355] text-white'
              : 'bg-[#2563EB] hover:bg-[#1D4ED8] text-white'
          }`}
        >
          <span className="hidden xs:inline">{currentStage.actionText}</span>
